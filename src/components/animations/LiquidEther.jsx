...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops) {
      let arr = Array.isArray(stops) && stops.length ? (stops.length === 1 ? [stops[0], stops[0]] : stops) : ["#fff", "#fff"]
      const w = arr.length
    ...
    // Shaders (recortado por brevedad – idénticos a tu snippet)
    // --- BEGIN SHADERS (copiados de tu snippet) ---
    ...
    const webgl = new WebGLManager({ $wrapper: container })
